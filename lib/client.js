...
`;var At="better-model-provider",ve="better-model-provider",Tt=["slots","locale","connection","remote"];function xe(e){e.store.getSnapshot().status!=="idle"&&e.reload()}function Mt(e){e.effect(()=>e.locale.register(ve,{zh:et,en:Ze}),"better-model-provider: dictionaries");let t=document.createElement("style");t.dataset.plugin="better-model-provider",t.textContent=tt,document.head.appendChild(t),e.effect(()=>()=>t.remove(),"better-model-provider: stylesheet");let n=new ee(e.connection.api);e.effect(()=>()=>n.dispose(),"better-model-provider: controller");let r=()=>(0,nt.useSyncExternalStore)(n.store.subscribe,n.store.getSnapshot),i=e.locale.bind(ve),o=(a,p)=>i(a,p),s=()=>({controller:n,useSnapshot:r,t:o});e.effect(()=>{let a=()=>{xe(n)},p=[e.remote.$on("settings/document-updated",d=>{d===W&&xe(n)}),e.remote.$on("llm/adapters-updated",a),e.on("connection/reset",a)];return()=>{for(let d of p)d()}},"better-model-provider: pushed invalidations"),e.slots.inject("settings.section",()=>e.slots.register({name:"settings.section",id:ve,order:11,label:()=>o("nav"),inject:s},we))}
